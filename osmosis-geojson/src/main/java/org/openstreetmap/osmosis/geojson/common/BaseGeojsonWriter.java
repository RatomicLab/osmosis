...
/**
 * An OSM data sink for storing all data to a geojson file.
 *
 * @author Brett Henderson
 */
public abstract class BaseGeojsonWriter {

    private static Logger log = Logger.getLogger(BaseGeojsonWriter.class.getName());
     ...
     */
    public BaseGeojsonWriter(BufferedWriter writer) {
        this.writer = writer;
     ...
     */
    public BaseGeojsonWriter(File file, CompressionMethod compressionMethod) {
        this.file = file;
     ...
     *
     * @param resultWriter
     *            The writer receiving geojson data.
     */
        ...
        try {
            ...
            if (!writerProvided) {
                try {
                    ...
                    throw new OsmosisRuntimeException("Unable to complete writing to the geojson stream.", e);
                } finally {
                    writer = null;
                }
            }
